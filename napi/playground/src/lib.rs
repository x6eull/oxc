...
impl Oxc {
    ...
    // /// Returns comments
    // /// # Errors
    // pub fn get_comments(&self) -> Vec<JsValue> {
    // self.comments
    // }

    /// # Errors
    pub fn run(&mut self, source_text: &str, options: OxcOptions) {
    ...
    fn run_linter(
        ...
        if run_options.lint.unwrap_or_default() && diagnostics.is_empty() {
    ...
    fn get_symbols_text(&self, scoping: &Scoping) {
        struct Data {
            span: Span,
            name: String,
            flags: SymbolFlags,
    ...
    fn save_diagnostics(&mut self, diagnostics: Vec<OxcDiagnostic>) {
        // self.diagnostics.extend(diagnostics.into_iter().map(OxcError::from));
    }
